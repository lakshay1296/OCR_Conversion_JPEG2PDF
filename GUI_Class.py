...
from Combined import perform_ocr, pdf2img, single_file_ocr
...
class window(object):
    ...
    def __init__(self, master):
        ...
        add_bar = Button(self.master, text="Add Bar", command=lambda: self.start_thread(var_list), bd=1, relief=SOLID) \
            .grid()
    ...
    def submit(self, var_list):
        ...
        if self.function == "Initiated fileopen":
            ...
            self.example_code(pdf_location=self.filename, command=self.commands)
    ...
    def start_thread(self, var_list):
        ...
        if var_list[1].get() is True:
            ...
            print("pass")
        t = Thread(target=lambda: self.example_code(pdf_location=self.filename, var_list=var_list, command=self.commands))
        t.start()

    def example_code(self, pdf_location,
